...
import { useGetPrestasiQuery } from '@/store/slices/pendaftaranAPI'
...
export function FormListPrestasi({
  name,
  headerLabel,
  placeholder,
  isDisabled,
  ...
  const { data, isSuccess, isLoading, isFetching } = useGetPrestasiQuery()
  ...
  let PrestasiOption = []
  if (isSuccess) {
    PrestasiOption = listPendidikan.map((item) => {
      return {
        value: item?.id,
        label: item?.nama,
      }
            ...
            <div className="w-4/6 phones:w-full">
              <FormControl>
                <Select
                  ...
                  className={'text-[2rem]'}
                  options={PrestasiOption}
                  value={
                    PrestasiOption.filter(

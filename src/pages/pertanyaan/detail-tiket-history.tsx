...
export function DetailHistory({ detail }: { detail: TiketDetailType }) {
  return (
    <div className="flex flex-col gap-32">
      {detail?.chat?.map((item, idx) => (
        <div
          className={`flex w-full gap-32 ${item?.jenis_chat === 'SISWA' ? 'justify-end' : 'flex-start'}`}
          key={idx}
        >
          <div
          ...
          >
            ...
            <div className="flex flex-1 flex-col gap-8 rounded-2xl border border-[#73C2FF] bg-[#f5faff] p-24">
              ...
              {detail?.ticket?.lampiran?.length > 0 && (
                <div className="grid grid-cols-12 gap-32">
                  {item?.lampiran?.map((list, index) => (
                    <div className="col-span-12 phones:col-span-12" key={index}>
                      <Link
                        to={list?.dokumen}
                        target="_blank"
